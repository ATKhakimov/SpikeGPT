...
В работе представлена адаптация импульсной языковой модели SpikeGPT к русскому языку. Модель основана на RWKV-подобной архитектуре с бинарными событийно-управляемыми LIF-нейронами: Spiking RWKV выполняет смешение информации по токенам, а Spiking Receptance Feed-Forward Network выполняет поканальное преобразование признаков. Такая структура согласуется с последовательной природой языкового моделирования и позволяет заменить квадратичную self-attention-рекуррентным механизмом, совместимым со спайковой динамикой. Обучена русскоязычная модель SpikeRuGPT размером около 100 млн параметров на корпусе «Тайга» объемом около 1,8 млрд токенов. Достигнута валидационная перплексия 59,79, что подтверждает применимость архитектуры SpikeGPT к морфологически богатому русскому языку. Выполнено сравнение нейроморфной спарсити русскоязычной модели с англоязычной SpikeGPT-OpenWebText-216M: средний firing rate для русского составил 33,2% против 21,7% для английского, то есть русский текст требует примерно на 53% больше спайков. Дополнительно исследована модификация LearnableLIF с обучаемыми параметрами мембранной постоянной времени и порога возбуждения; показано формирование иерархии временных масштабов по слоям модели.
...
This paper presents an adaptation of the spiking language model SpikeGPT to Russian. The model is based on an RWKV-like architecture with binary event-driven LIF neurons: Spiking RWKV performs token mixing, while the Spiking Receptance Feed-Forward Network performs channel mixing. This structure matches the sequential nature of language modeling and replaces quadratic self-attention with a recurrent mechanism compatible with spiking dynamics. A Russian-language SpikeRuGPT model with approximately 100 million parameters is trained on the Taiga corpus containing about 1.8 billion tokens. The model reaches a validation perplexity of 59.79, confirming the applicability of the SpikeGPT architecture to morphologically rich Russian text. We compare neuromorphic sparsity of the Russian model with SpikeGPT-OpenWebText-216M: the average firing rate is 33.2% for Russian versus 21.7% for English, meaning that Russian text requires about 53% more spikes. Additionally, we investigate a LearnableLIF modification with trainable membrane time constants and firing thresholds, showing the formation of a layer-wise hierarchy of temporal scales.
...
## 7. Дополнительная серия экспериментов

После обучения первой версии модели была начата новая серия экспериментов, направленная на повышение воспроизводимости и параметрической эффективности. В ней используется меньший SentencePiece BPE tokenizer на 32 тыс. токенов и более строгая фильтрация корпуса. Такая версия имеет около 74M параметров, сохраняя 12 слоев и размер скрытого состояния 512.
...
Таблица 5. Сравнение плотности tokenizer-ов на общем русском validation-фрагменте.

Промежуточные checkpoint-ы новой модели показывают положительную динамику: validation loss снижается, а средний firing rate уменьшается, что можно интерпретировать как формирование более разреженного внутреннего представления.
...
Таблица 6. Промежуточная динамика новой версии модели.

Эти результаты не заменяют основной эксперимент с моделью SpikeRuGPT 100M, но показывают направление дальнейшего развития: более компактный tokenizer, более чистый корпус и анализ спайковой активности как регулярной метрики обучения.
...
Ограничения работы также существенны. Сравнение русской и английской моделей проводится между моделями разного размера и на разных корпусах, поэтому его следует рассматривать как практическое наблюдение, а не как окончательное доказательство универсальной языковой закономерности. Кроме того, модель не проходила instruction tuning и не предназначена для factual QA без дополнительного обучения.

## 9. Заключение

В работе обучена русскоязычная импульсная языковая модель SpikeRuGPT на архитектуре SpikeGPT. Модель размером около 100M параметров, обученная на корпусе «Тайга» объемом около 1,8 млрд токенов, достигла validation perplexity 59,79 и продемонстрировала способность генерировать связный русский текст.
...
Дополнительно исследована LearnableLIF-модификация с обучаемыми параметрами τ и θ. В экспериментах наблюдается рост τ с глубиной слоя, что подтверждает гипотезу об иерархии временных масштабов в спайковой языковой модели. Дальнейшая работа включает доменное дообучение на технических корпусах, полное обучение модели с LearnableLIF, оценку на русскоязычных бенчмарках и измерение реального энергопотребления на нейроморфном аппаратном обеспечении.
